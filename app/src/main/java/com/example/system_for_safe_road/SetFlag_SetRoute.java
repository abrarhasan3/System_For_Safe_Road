...
public class SetFlag_SetRoute extends AppCompatActivity implements OnMapReadyCallback, RoutingListener {
    ...
    private void save_in_firebase() {
        ...
        while(i<hashsize){
            ...
            databaseReference.child(routeID).child("Flag").child("Flag"+i).child("n_index").setValue(i+1);
            databaseReference.child(routeID).child("Flag").child("Flag"+i).child("p_index").setValue(i-1);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                Intent intent = new Intent(SetFlag_SetRoute.this, adminPannel.class);
